...
test("uses model-specific thinking level maps without disabling reasoning-capable CommandCode models", () => {
  ...
  assert.deepEqual(byId.get("deepseek/deepseek-v4-pro")?.thinkingLevelMap, { off: "off", minimal: "minimal", low: "low", medium: "medium", high: "high", xhigh: null });
  assert.deepEqual(byId.get("Qwen/Qwen3.6-Plus")?.thinkingLevelMap, { off: "off", minimal: "minimal", low: "low", medium: "medium", high: "high", xhigh: null });
  assert.deepEqual(byId.get("gpt-5.5")?.thinkingLevelMap, { off: "none", minimal: "minimal", low: "low", medium: "medium", high: "high", xhigh: null });
  assert.deepEqual(byId.get("claude-sonnet-4-6")?.thinkingLevelMap, { off: "disabled", minimal: "low", low: "low", medium: "medium", high: "high", xhigh: null });
  assert.deepEqual(byId.get("claude-opus-4-7")?.thinkingLevelMap, { off: "disabled", minimal: "low", low: "low", medium: "medium", high: "high", xhigh: "xhigh" });
  assert.deepEqual(byId.get("claude-opus-4-6")?.thinkingLevelMap, { off: "disabled", minimal: "low", low: "low", medium: "medium", high: "high", xhigh: "max" });
});
...
test("uses CommandCode pricing docs for bundled model costs", () => {
  ...
  assert.deepEqual(byId.get("deepseek/deepseek-v4-pro")?.cost, { input: 0.435, output: 0.87, cacheRead: 0.003625, cacheWrite: 0 });
